  ...
  return robot.respond(/.*/i, function(res) {
    ...
    return res.http('https://api.apigw.smt.docomo.ne.jp/dialogue/v1/dialogue').header('Content-Type', 'application/json').query({
      ...
      if (err != null) {
        ...
        var say = data.utt;
        say = say.replace(/ー{1,2}/,"");
        say = say.replace(/んです.{0,1}$/, 'の').replace(/です.{0,1}$|ですね.{0,1}$|そうです.{0,1}$|そうですね.{0,1}$|そうですよ.{0,1}$/, 'よ').replace(/[の|ん]ですが/, "んだけど");

        if(say.match(/[。|！|？]$/)){
          say = say.replace(/[。|？|！]{1,3}/, function(a){return a.substr(0,1)}).replace(/！/g, '...').replace(/[。|？]$/, function(a){ return "..."+a});
        }else{
          say = say + "...";
        };
